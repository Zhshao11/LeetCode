...
- 2.LocalDatatime是什么类？
	- LocalDataTime是一个不可变的，线程安全的日期时间对象
	- 在实体类里通过@CreationTimestamp注解可以在第一次执行Insert时给LocalDataTime变量赋值
#### 3.数据访问层实现：
##### 数据访问层（DAO层）的技术选型（持久化框架选型）
- Q：
	- 什么是ORM？
	- 技术选型：Mybatis/Mybatis-Plus?还是SpringData JPA/Hibernate
	- 使用的是SpringData JPA，底层由Hibernate驱动，为什么？（JPA，Hibernate，SpringData JPA的关系）
	- JPA和Hibernate的各个注解含义：`@Entity、@Table、@Id、@GeneratedValue、@Column、@Enumerated、@CreationTimestamp、@UpdateTimestamp`
 - A：
	- ORM是通过实例对象的语法，完成关系型数据库的操作的技术是（Object Relational Mapping）对象-关系映射的缩写
		- 为什么可以实现对象关系映射？----面向对象变成是把实体看成对象，而关系型数据库是采用实体之间的关系连接数据
	- JPA、Hibernate、Spring Data JPA的区别和联系：
		- JPA是一套JavaEE的一套规范（是一堆接口和注解的定义），但没有具体的实现
		- Hibernate是对JPA的具体实现，是一套**全自动的ORM框架**，负责在底层把Java对象转为SQL语句去操作数据库；
		- Spring Data JPA是基于JPA规范，底层封装了Hibernate做的一层高级抽象和封装
	- 技术选型：Mybatis/Mybatis-Plus?还是SpringData JPA/Hibernate
		- Mybatis/Mybatis-Plus是半自动ORM，JPA是全自动ORM
		- 前者是以SQL为中心，JPA是以对象为中心
		- 开发效率：前者单表CRUD较慢，复杂查询较快，JPA相反
		- SQL优化：前者较方便，后者较困难
		- 数据库移植性：前者较差，后者极好
	- JPA和Hibernate的各个注解含义：`@Entity、@Table、@Id、@GeneratedValue、@Column、@Enumerated、@CreationTimestamp、@UpdateTimestamp`
		- JPA注解：
			...
			- 5.Column：用于映射数据库表的列，可以设置name、length、nullable、unique
			- 6.Enumerated：映射枚举类型到数据库
		- Hibernate注解：
			- 1.CreationTimestamp：程序第一次执行Insert，就把当前时间赋值给属性
			- 2.UpdateTimestamp：程序update，就把当前时间赋值给属性
...
#### 4.单元测试
- Q：@SpringBootTest、@Autowired注解原理？
- 注册Bean
	- 为什么实体接口不用注册Bean？
...
- 创建实体类
  - @Column注解里的(columnDefinition = "TEXT")什么意思？
    - JPA会默认把String映射为VARCHAR(255)，对于description来说，这不够，所以指定SQL数据类型："TEXT"
...
- 1.校验：
  ...
  - elasticSearch删除：
    - DeleteByQueryRequest request = DeleteByQueryRequest.of(d -> d
      ...
      esClient.deleteByQuery(request);语法是什么？
### 获取用户可访问的全部文件列表接口
- 1.校验：用户是否存在
- 2.查数据库
  - 1.自己上传的，所属id为自己
  - 2.表里is_public为1的
  - 3.遍历orgTags，查所有包含orgTag的文件
    - 这里遍历的是有效的orgTags,有效orgTags怎么来的，怎么维护
- 问题：如果有重复文件怎么办
  - 直接一个SELECT 语句就完成了
### 获取用户上传的文件列表接口
- 问题：明明一个语句就解决了，为什么要定义一个方法？
- 返回值有一个值和fileUpload的DTO定义不同，需要转为返回值的dto
### 解析文档接口("/api/v1/parse")
#### 文件解析服务
- 1.文件解析配置：每个文本块最大字符数，缓冲区，内存阈值
- 2.检查内存
  - checkMemoryThreshold：检查当前内存使用率，totalMemory-freeMemory/maxMemory，使用率高于0.8触发gc机制。因为文档解析非常消耗内存，通过检查JVM的剩余内存防止当前解析任务吧整个应用拖垮。
- 3.基于Apache Tika的流式文档解析（buffer）与分块入库
  - 1.Metadata:用于存放文档元数据
  - 2.ParseContext:解析上下文，用于向Tika传递一些高级配置
  - 3.AutoDetectParser:自动探测文件类型，并智能选择对应的底层解析引擎
  - 观察者模式是什么?
  - 定义内部流式内容处理器StreamingContentHandler
    - 构造函数里super(-1)禁用Tika的内部写入限制
    - Tika读到一点文字就调用一次characters方法
      - 里面定义了chunkSize
    - 解析到文件末尾调用endDocument
    - 核心处理逻辑（processParentChunk）：
      - 1.对父块进行智能文本分割，保证在句号，换行符处截断，或者给每段话增加一定重叠度
        - 智能文本分割：
          - 1.按照段落分；
          - 2.按照句子分
          - 3.使用HanLP按语义分割；备用按字符分(对长词按照chunkSize进行分割)
      - 2.保存分片到document_vectors数据库
    - 整个流程是：Tika调用characters方法，characters是把文本累加到一起，当达到父块大小的时候就执行processParentChunk；对父块进行语义切分String--->List<String>然后把分片数组持久化,持久化完成后清空缓冲区
  - 问题是，Tika怎么实现在文档的哪里调用哪个函数？
- InputStream是什么？
- 流式处理器？
- BufferedInputStream？
- Metadata？ParseContext？AutoDetectParser？Tika的parse方法？
- BufferedInputStream bufferedStream = new BufferedInputStream(fileStream, bufferSize)是什么
# 知识库检索模块设计
- 1.entity包和model包区别？哪些要写到entity哪些要写到model
- 不带userId的检索（普通检索）
  - 1.生成查询向量
    - 1.texts从String转为List<String>发生什么变化
    - 2.在使用之前要进行配置
      - @Configuration：项目启动时先读取这里的设置
      - 1.strategies调整Spring WebFlux大小，默认为256KB，调整为16MB
      - 2.封装公共属性
        - 1.baseurl;2.strategies(16MB内存限制策略)3.全局默认请求头：鉴权头(Bearer+api Key)；请求内容类型:application/json
    - 3.流程：
      - 1.List<String>输入文本，然后按照batchSize切片，然后组装成请求体通过Spring的WebClient进行HTTP调用，获取到JSON响应，将JSON中的data[].embedding转为float[]，再汇总拼接返回List<float[]>
        - 请求体在java中构建为Map，Map在传入WebClient时会被序列化为json，类似接收响应时将json转为java对象(这里为String)
        - 其中涉及通过Spring的WebClient进行HTTP调用：
          - ```
            webClient.post() // 1. 指定 HTTP 方法
            .uri("/embeddings") // 2. 指定路由
            .bodyValue(requestBody) // 3. 填充请求体
            .retrieve() // 4. 发送并获取响应
            .bodyToMono(String.class) // 5. 反序列化
            .retryWhen(Retry.fixedDelay(3, Duration.ofSeconds(1))
            .filter(e -> e instanceof WebClientResponseException)) // 6. 重试策略
            .block(Duration.ofSeconds(30)); // 7. 阻塞并设置超时
            ```
          - 这是链式调用（fluent api）:
            - post表示是一个HTTP POST请求，
            - uri表示请求的具体路径，
            - bodyValue表示请求体，
            - retrieve表示正式触发请求并声明要对响应内容进行提取，还有exchange()，是返回包含状态码、响应头的完整响应对象
            - bodyToMono表示要将HTTP 响应体反序列化为指定的 Java 类型(String.class),
            - retryWhen表示异常重试机制，Retry.fixedDelay(3, Duration.ofSeconds(1))是固定延迟重试，最多重试3次，每次间隔1秒，
            - .filter(e -> e instanceof WebClientResponseException)重试条件过滤，只有异常是HTTP响应异常才重试，
            - .block将异步的响应式调用强制转为同步阻塞调用，并设置超时时间
    - 除了WebClient还有什么方法？
      - 还有RestTemplate、RestClient、Spring HTTP Interfaces/OpenFeign,java.net.http.HttpClient,OkHttp,Apache HttpClient
  - 生成失败使用文本匹配
    - 文本匹配就是关键词检索： s.query(q -> q.match(m -> m.field("textContent").query(query)));
  - 2.检索：
    - ```
      ...
      ```
    - 扩大初始召回池：int recallK = topK * 30;
    - 向量检索：
      - field定位到目标字段
      - queryVector传入查询向量
      - k：最终返回结果的数量
      - numCandidates：每个分片取recallK个然后对k*分片个数的结果进行再排序，最后返回k个
    - 关键词匹配：
      - s.query(q -> q.match(m -> m.field("textContent").query(query)));关键词匹配默认为OR规则：比如搜索：今天下雨：分词器分为今天和下雨，只要包含这两个词的都会被检索出来
    - 打分重排：
      - windowSize：重排打分的区域
      - query:这次查询具体操作
      - queryWeight(0.2d)前面的打分是赋予20%权重，rescoreQueryWeight(1.0d)这次打分是100%权重
      - query里面的query是这次检索的规则，代表着AND规则的关键词过滤，例如今天下雨只有今天和下雨同时出现才会检索到
  - 3.返回：
    - ```
...
- 带权限(userId)的检索
  - 1.获取有效组织标签
    - 有效组织标签是什么意思 
    - 1.从缓存获取(有效组织标签哪里来的)
    - 2.缓存未命中从数据库获取(自己的标签加上递归得到的所有父标签)，为什么父标签也参与检索
    - 3.默认标签怎么不加username了
    - 4.rightPushAll含义，以及递归的时候collectParentTags没有返回值，为什么值能回到主程序
  - 2.数据库ID是什么意思？
    - 意思不管传来的是用户ID还是用户登录名，统一返回数据库中用户ID
  - 3.带权限的文本搜索
    - 1.filter过滤：1.userId的文档可以通过2.public的文档可以通过3.组织标签里的文档可以通过
    - 2.must进行文本匹配，里面指定textContent字段，ES根据查询的词去textContent字段里进行比对打分，贴到文档上进行返回
    - 3.打分策略：BM25算法
  - 4.带权限的检索：
    - 1.向量检索；2.带权限的文本匹配；3.得分重排
    - 为什么向量检索不用权限过滤？
  - 5.将文件名添加到result里，首先根据fileMd5的set集合查询fileUpload表（在SQL里执行IN查询），不能使用for循环来查，会产生N+1查询问题
    - 什么是N+1查询问题？
- ES会返回什么？返回的格式是怎样的，是ES存的时候确定的吗
# 文件处理消费者
- 1.kafka设置
  - 1.启用kafka
  - 2.设置kafka服务地址
  - 3.生产者设置：这在文件上传部分介绍过
  - 4.消费者设置：
    - 消费者组ID；auto-offset-reset；key-deserializer（key反序列化）；value-deserializer（value反序列化）；properties:spring.json.trusted.packages允许反序列化的包和client.dns.lookup；topic：新增的topic设置和死信队列主题。
- 2.监听kafka(注解里指定topics和groupId)
  - 下载文件，返回值是InputStream类型的
    - 通过下载函数的filePath来下载
      - 如果是文件系统路径返回的是FileInputStream(file)是什么意思
      - 如果是URL就通过HttpURLConnection来设置请求方法，连接超时时间，读取超时时间，请求头，最后getInputStream（之前要判断responseCode是否是HTTP_OK）
  - 转换为可缓存流
  - 解析文件
    - 输入参数：fileMd5,userId，isPublic,orgTag还有文件输入流
    - 调用文件解析服务进行解析：判断memory剩余，调用tika解析，tika调用流式文档处理器进行父块（父块大小设置为）分片解析，包括按段分、按句分、按词分（备用使用chunkSize截断）
  - 向量化处理
    - 通过fileMd5获得文件分块内容；
    - 提取文本内容并调用外部模型生成向量；
    - 最后构建es文档并存储
      - 新建EsDocument类,分片赋值，批量存到es
      - 批量存储到ES:
        - 将文档列表转换为批量操作列表（操作是将文档列表中的id作为索引，数据是文档对象），然后封装成请求给esClient.bulk，会返回BulkResponse，里面包含了每个操作的结果，最后需要遍历这个结果，对存储失败的结果写日志。（ES存储是部分成功机制）
      - 问题：最后的es里的存储单元是chunk吗
  - 最后关闭输入流
- 问题：发送的task包含哪些信息？
- 什么时候要 private final ParseService parseService;什么时候要自动注入